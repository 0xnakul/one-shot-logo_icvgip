...
parser.add_argument('--epochs',     type=int,   default=402,           help='Training epochs')
...
loss_fn = conLossv3(TEMP)

result_path = 'txt-results_' + args.arch + '_' + args.dataset + '_hlr_' + str(args.lr) + '_temp_' + str(TEMP)
...
tr_loader = data_loader(data_path, args.exp, is_transform=True, split='train', img_size=(args.img_rows, args.img_cols), use_txt=True)
te_loader = data_loader(data_path, args.exp, is_transform=True, split='test', img_size=(args.img_rows, args.img_cols), use_txt=True)
val_loader = data_loader(data_path, args.exp, is_transform=True, split='val', img_size=(args.img_rows, args.img_cols), use_txt=True)
...
model = get_model(args.arch, n_classes=None, txt_dim=256)
for i, k in enumerate(model):
    model[i] = k.cuda()


def net(img, txt, train=False):
    x = model[0](img)
    x = torch.cat([x, txt], -1)
    if train:
        x = model[1](x)
...
def train(e):
    ...
    for i, data in enumerate(trainloader):
        ...
        jpg_txt = data['jpg_txt']
        templ1, templ2 = data['templ1'], data['templ2']
        templ_txt = data['templ_txt']
        view1 = torch.cat([view1, templ1]).cuda(non_blocking=True)
        view2 = torch.cat([view2, templ2]).cuda(non_blocking=True)
        txt = torch.cat([jpg_txt, templ_txt]).cuda(non_blocking=True)

        x, y = net(view1, txt, train=True), net(view2, txt, train=True)
    ...
    if e%save_epoch == 0:
        ...
        class_template, _ = tr_loader.load_template(class_target)
...
def test(e, best_acc, val_trigger):
    ...
    class_target = torch.LongTensor(list(range(n_classes)))
    class_template, templ_txt = te_loader.load_template(class_target)
    class_template = class_template.cuda(non_blocking=True)
    templ_txt = templ_txt.cuda(non_blocking=True)
    with torch.no_grad():
        class_mu = net(class_template, templ_txt)
    
    for i, data in enumerate(testloader):

        target = data['label']
        input = data['img']
        txt = data['jpg_txt'].cuda(non_blocking=True)
...
def validation(e, best_acc):
    ...
    # get template latent z
    class_target = torch.LongTensor(list(range(n_classes)))
    class_template, templ_txt = val_loader.load_template(class_target)
    class_template = class_template.cuda(non_blocking=True)
    templ_txt = templ_txt.cuda(non_blocking=True)
    with torch.no_grad():
        class_mu = net(class_template, templ_txt)
